...
const cube collectOrder[2] = {BIGBLACK, SMALLBLACK}; // массив порядка сбора кубиков ( определяется жеребьёвкой )
...
const int stepFor90Rotation = 125;//170; // количество шагов для поворота 90 градусов
const int stepFor90RotationCube = 125;//170; // количество шагов для поворота 90 градусов когда есть кубики
...
const int smallCubeLimit = 54; // порог значения с детектора кубика, после которого считаем, что кубик маленький
...
void setup() {
  ...
  delay(300);
  servoPullPulling(); // выдвинуть сервопривод захвата
  delay(300);
...
{
  pwm.setPWM(SERVO_PULL, 0, pulseWidth(39));
}

// отпускание вниз выталкивающего сервопривода
void servoPushRelease()
{
  pwm.setPWM(SERVO_PUSH, 0, pulseWidth(15));
}
...
void move90Right()
{
  ...
  for (int x = 0; x < stepBetweenSensorAndMotor; x++) { //немного поехать вперёд
    leftMotor.step(1);
    rightMotor.step(1);
  }
  ...
  if (countPulledCube == 0) // если нет затянутых кубиков
    for (int x = 0; x < stepFor90Rotation; x++) { // поворот на месте
      leftMotor.step(1);
      rightMotor.step(-1);
    }
  else // если есть затянутые кубики
    for (int x = 0; x < stepFor90RotationCube; x++) { // поворот на месте
      leftMotor.step(1);
      rightMotor.step(-1);
    }
  leftMotor.setSpeed(stepperspeed);
...
void move90Left()
{
  ...
  if (countPulledCube == 0) // если нету затянутых кубиков
    for (int x = 0; x < stepFor90Rotation; x++) { // поворот на месте
      leftMotor.step(-1);
      rightMotor.step(1);
    }
...
void moveFromBase()
{
  for (int x = 0; x < 430; x++) { // выехать вперёд и на сколько
    leftMotor.step(1);
    rightMotor.step(1);
  }
  move90Left(); // повернуть налево
}
...
void loop() {
  ...
  )
  {
    ...
    if (countPulledCube > 0) // если есть затянутые кубики
    {
      ...
      leftMotor.setSpeed(50);
      rightMotor.setSpeed(50);
      //move90Right(); // поворачиваемся направо
      for (int x = 0; x < stepFor90RotationCube; x++) { // поворот на месте
        leftMotor.step(1);
        rightMotor.step(-1);
      }
      for (int x = 0; x < 400; x++) { // едем вперёд
        leftMotor.step(1);
        rightMotor.step(1);
      }
      servoPullRelease(); // сбрасываем кубики
      for (int x = 0; x < 400; x++) { // едем назад
        leftMotor.step(-1);
        rightMotor.step(-1);
      }
      //move90Left(); // поворачиваемся налево
      for (int x = 0; x < stepFor90RotationCube; x++) { // поворот на месте
        leftMotor.step(-1);
        rightMotor.step(1);
      }
